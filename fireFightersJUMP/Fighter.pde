...
void quickAttack(){
    ...
    if(playerNumber == 1 && ani1.pDoinIt == false && player1.location.y == 800){
    ...
    if(playerNumber == 2 && ani2.pDoinIt == false && player2.location.y == 800){
      ani2.counter = frameCount;
      ani2.pAnimation = "QuickAttack";
      sfx.playSound("Kick1.wav");
      rect(location.x-qAWidth,location.y,qAWidth,qAHeight);
      if((location.x-2*qAWidth<player1.location.x)&&(location.y-qAHeight-fighterHeight<player1.location.y)){
        println("p1 hit");
        lifePlayer1 -= 20;
